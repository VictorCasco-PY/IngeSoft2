...
const EntrenamientoIntermedio = () => {
  ...
  const { getProgramasIntermedio, getProgramasByActividad, getActividades } = usePlanes();
  ...
  const handleSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await getProgramasIntermedio(1, search); // Aquí pasa el parámetro de búsqueda
      setData(res);
